...
contract Auction{
    ...
    function bid() payable public notOwner started beforeEnded{
        ...
        require(currentBid>highestPaybleBid);
        bids[msg.sender]=currentBid;
        if(currentBid<bids[highestBidder]){
            highestPaybleBid= min(currentBid+bidInc,bids[highestBidder]);
        }else{
            highestPaybleBid= min(currentBid,bids[highestBidder]+bidInc);
    ...
    function finalizeAuc() public{
        ...
        if(auctionState==Auc_State.Cancelled){
            ...
            if(msg.sender==auctioneer){
                person=auctioneer;
                value= highestPaybleBid;
            }else{
                if(msg.sender==highestBidder){
                    person=highestBidder;
                    value=bids[highestBidder] - highestPaybleBid;
                }else{
                    person=payable(msg.sender);
                    value=bids[msg.sender];
                }
